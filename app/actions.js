...
export async function fetchExamQuestions() {
  ...
  try {
    const { data, error } = await supabase
      .from('exam_questions')
      .select(
        'id, section_name, question_type, question_text, options, correct_answers, points'
      )
    ...
    return {
      success: true,
      examSections: Object.entries(groupedQuestions).map(
  ...
  try {
    ...
    examQuestions.forEach((question) => {
      ...
      if (question.question_type === 'written') {
        ...
        const incorrectCount = userAnswerArray.filter(
        ...
        const correctAnswer = Array.isArray(question.correct_answers)
          ...
          : JSON.parse(question.correct_answers || '[]')[0]
