...
export default function Home() {
  ...
  const validateInputs = () => {
    ...
    if (!phoneNumberRegex.test(phoneNumber)) {
      toast.error(language === 'ar' ? 'رقم هاتف غير صالح. يجب أن يكون رقمًا مكونًا من 10 أرقام يبدأ بـ 07' : 'Invalid phone number. It should be a 10-digit number starting with 07');
      return false;
    }
    if (!idNumberRegex.test(idNumber)) {
      toast.error(language === 'ar' ? 'رقم هوية غير صالح. يجب أن يكون مكونًا من 10 أرقام بالضبط' : 'Invalid ID number. It should be exactly 10 digits');
      return false;
    }
    return true;
  };
  ...
  const handleClick = async () => {
    try {
      ...
      toast.error(language === 'ar' ? `خطأ في تسجيل الدخول: ${error.message}` : `Login error: ${error.message}`);
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleAdminSubmit = () => {
    if (!adminEmail || !name) {
      toast.error(language === 'ar' ? 'يرجى إدخال البريد الإلكتروني واسمك' : 'Please enter your email and name');
      return;
    }
    
    localStorage.setItem('adminEmail', adminEmail);
    toast.success(language === 'ar' ? 'تم تسجيل دخول المسؤول بنجاح' : 'Admin login successful');
    router.push('/admin');
  };
